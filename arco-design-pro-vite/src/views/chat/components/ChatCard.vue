...
<script lang="ts" setup>
  ...
  function handleScroll(e: Event) {
    ...
    if (scrollTop > preScrollTop && bottom <= 0) emit('reachBottom');
  ...
  const contentCls = computed(() => [`${prefixCls}`, [`${prefixCls}-virtual`]]);
...
<template>
  <div :class="`${prefixCls}-wrapper`">
    <a-spin :class="`${prefixCls}-spin`" :loading="props.loading">
      <a-scrollbar
        ref="componentRef"
        :class="cls"
        :style="contentStyle"
